...
public class Ecran extends Application {
  ...
  //      scene.getStylesheets().add(getClass().getResource("resources/style.css")
                //.toExternalForm());
    ...
    private GridPane createDashboard() {
        ...
        dashboard.add(productsSection, 0, 0);

        GridPane.setVgrow(productsSection, Priority.ALWAYS);
        GridPane.setHgrow(productsSection, Priority.ALWAYS);
    ...
    private VBox createProductsSection() {
        VBox productsBox = createSection("Produits");
        ...
        productsBox.getChildren().add(tableView);
        VBox.setVgrow(productsBox, Priority.ALWAYS);

        HBox buttonBox = createButtonBox();
        productsBox.getChildren().add(buttonBox);
    ...
    private void showAddProductDialog() {
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == addButtonType) {
                ...
                switch (type) {
                    case "Alimentaire":
                        return new Alimentaire(idField.getText(), nomField.getText(), Double.parseDouble(prixField.getText()), Integer.parseInt(quantiteField.getText()), "2024-08-02");
                    case "Electronique":
                        return new Electronique(idField.getText(), nomField.getText(), Double.parseDouble(prixField.getText()), Integer.parseInt(quantiteField.getText()), "Marque");
                    case "Vestimentaire":
                        return new Vestimentaire(idField.getText(), nomField.getText(), Double.parseDouble(prixField.getText()), Integer.parseInt(quantiteField.getText()), "Taille");
                    default:
                        return null;
                }
            }
    ...
    private void showDeleteProductDialog() {
        ...
        dialog.showAndWait().ifPresent(id -> {
            Produit produit = magasin.getProduitById(id);
            if (produit != null) {
